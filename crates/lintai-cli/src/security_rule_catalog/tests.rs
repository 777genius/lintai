...
fn shipped_rules_have_expected_default_preset_mapping() {
    ...
    let sec102 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC102")
        .unwrap();
    assert_eq!(sec102.default_presets(), vec!["threat-review", "skills"]);
    assert_eq!(sec102.public_lane(), PublicLane::ThreatReview);
    assert_eq!(sec102.metadata.category, Category::Audit);

    let sec103 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC103")
        .unwrap();
    assert_eq!(sec103.default_presets(), vec!["threat-review", "skills"]);
    assert_eq!(sec103.public_lane(), PublicLane::ThreatReview);
    assert_eq!(sec103.metadata.category, Category::Audit);

    let sec104 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC104")
        .unwrap();
    assert_eq!(sec104.default_presets(), vec!["threat-review", "skills"]);
    assert_eq!(sec104.public_lane(), PublicLane::ThreatReview);
    assert_eq!(sec104.metadata.category, Category::Audit);
    ...
    let sec350 = entries
        ...
        .unwrap();
    assert_eq!(sec350.default_presets(), vec!["threat-review", "skills"]);
    assert_eq!(sec350.public_lane(), PublicLane::ThreatReview);
    assert_eq!(sec350.metadata.category, Category::Audit);

    let sec351 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC351")
        .unwrap();
    assert_eq!(sec351.default_presets(), vec!["threat-review", "skills"]);
    assert_eq!(sec351.public_lane(), PublicLane::ThreatReview);
    assert_eq!(sec351.metadata.category, Category::Audit);
    ...
    assert_eq!(sec319.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec302.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec304.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec331.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec342.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec365.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec366.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec448.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec462.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec337.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec343.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec345.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec346.metadata.category, Category::Hardening);
    ...
    let sec744 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC744")
        .unwrap();
    assert_eq!(sec744.default_presets(), vec!["supply-chain"]);
    assert_eq!(sec744.public_lane(), PublicLane::SupplyChain);
    assert_eq!(sec744.metadata.category, Category::Hardening);

    let sec745 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC745")
        .unwrap();
    assert_eq!(sec745.default_presets(), vec!["supply-chain"]);
    assert_eq!(sec745.public_lane(), PublicLane::SupplyChain);
    assert_eq!(sec745.metadata.category, Category::Hardening);

    let sec753 = entries
        .iter()
        .find(|entry| entry.metadata.code == "SEC753")
        .unwrap();
    assert_eq!(sec753.default_presets(), vec!["supply-chain"]);
    assert_eq!(sec753.public_lane(), PublicLane::SupplyChain);
    assert_eq!(sec753.metadata.category, Category::Hardening);
    ...
    assert_eq!(sec306.metadata.category, Category::Audit);
