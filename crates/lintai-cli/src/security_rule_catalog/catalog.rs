...
pub(super) fn render_preset_activation_model() -> Vec<String> {
    vec![
        ...
        "- `governance`: opt-in review rules for shared mutation authority and broad bare tool grants that should stay separate from the main security lane".to_owned(),
        "- `supply-chain`: sidecar supply-chain hardening checks such as GitHub Actions workflow rules".to_owned(),
        "- `advisory`: offline dependency vulnerability checks driven by installed lockfile versions and the active advisory snapshot".to_owned(),
        String::new(),
