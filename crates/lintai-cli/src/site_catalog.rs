...
mod tests {
    ...
    fn representative_preset_memberships_match_expectations() {
        ...
        let sec314 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC314")
            .expect("SEC314 should exist");
        assert_eq!(sec314.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec314.public_lane, "compat");

        let sec315 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC315")
            .expect("SEC315 should exist");
        assert_eq!(sec315.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec315.public_lane, "compat");

        let sec316 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC316")
            .expect("SEC316 should exist");
        assert_eq!(sec316.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec316.public_lane, "compat");

        let sec317 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC317")
            .expect("SEC317 should exist");
        assert_eq!(sec317.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec317.public_lane, "compat");

        let sec318 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC318")
            .expect("SEC318 should exist");
        assert_eq!(sec318.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec318.public_lane, "compat");
        ...
        let sec342 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC342")
            .expect("SEC342 should exist");
        assert_eq!(sec342.default_presets, vec!["supply-chain", "claude"]);
        assert_eq!(sec342.public_lane, "supply-chain");

        let sec364 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC364")
            .expect("SEC364 should exist");
        assert_eq!(sec364.default_presets, vec!["governance", "claude"]);
        assert_eq!(sec364.public_lane, "governance");

        let sec365 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC365")
            .expect("SEC365 should exist");
        assert_eq!(sec365.default_presets, vec!["supply-chain", "claude"]);
        assert_eq!(sec365.public_lane, "supply-chain");

        let sec366 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC366")
            .expect("SEC366 should exist");
        ...
        let sec303 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC303")
            .expect("SEC303 should exist");
        assert_eq!(sec303.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec303.public_lane, "governance");

        let sec307 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC307")
            .expect("SEC307 should exist");
        assert_eq!(sec307.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec307.public_lane, "governance");

        let sec311 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC311")
            .expect("SEC311 should exist");
        assert_eq!(sec311.default_presets, vec!["compat", "mcp"]);
        assert_eq!(sec311.public_lane, "compat");

        let sec336 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC336")
            .expect("SEC336 should exist");
        assert_eq!(sec336.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec336.public_lane, "governance");

        let sec330 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC330")
            .expect("SEC330 should exist");
        assert_eq!(sec330.default_presets, vec!["supply-chain", "mcp"]);
        assert_eq!(sec330.public_lane, "supply-chain");

        let sec341 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC341")
            .expect("SEC341 should exist");
        assert_eq!(sec341.default_presets, vec!["supply-chain", "claude"]);
        assert_eq!(sec341.public_lane, "supply-chain");

        let sec344 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC344")
            .expect("SEC344 should exist");
        assert_eq!(sec344.default_presets, vec!["supply-chain", "mcp"]);
        assert_eq!(sec344.public_lane, "supply-chain");
        ...
        let sec394 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC394")
            .expect("SEC394 should exist");
        assert_eq!(sec394.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec394.public_lane, "governance");

        let sec397 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC397")
            .expect("SEC397 should exist");
        assert_eq!(sec397.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec397.public_lane, "governance");

        let sec546 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC546")
            .expect("SEC546 should exist");
        assert_eq!(sec546.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec546.public_lane, "governance");

        let sec625 = catalog
            .rules
            .iter()
            .find(|rule| rule.rule_id == "lintai-ai-security:SEC625")
            .expect("SEC625 should exist");
        assert_eq!(sec625.default_presets, vec!["governance", "mcp"]);
        assert_eq!(sec625.public_lane, "governance");
