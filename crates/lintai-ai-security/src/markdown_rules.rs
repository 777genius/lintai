...
pub(crate) fn check_markdown_git_http_clone(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.git_http_clone_spans.as_slice())
            .unwrap_or(&[]),
        "AI-native markdown clones a Git repository from an insecure `http://` source",
    )
}

pub(crate) fn check_markdown_git_http_remote(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.git_http_remote_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_markdown_git_ssl_no_verify(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.git_ssl_no_verify_spans.as_slice())
            .unwrap_or(&[]),
        "AI-native markdown disables Git TLS verification with `GIT_SSL_NO_VERIFY`",
    )
}

pub(crate) fn check_markdown_git_inline_sslverify_false(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.git_inline_sslverify_false_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_wildcard_bash_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.wildcard_bash_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_gh_api_post_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_api_post_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh api --method POST:*)` tool access",
    )
}

pub(crate) fn check_gh_issue_create_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_issue_create_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh issue create:*)` tool access",
    )
}

pub(crate) fn check_gh_api_delete_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_api_delete_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh api --method DELETE:*)` tool access",
    )
}

pub(crate) fn check_gh_api_patch_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_api_patch_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh api --method PATCH:*)` tool access",
    )
}

pub(crate) fn check_gh_api_put_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_api_put_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh api --method PUT:*)` tool access",
    )
}

pub(crate) fn check_gh_repo_create_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_repo_create_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_gh_repo_edit_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_repo_edit_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_gh_release_create_allowed_tools(
    ...
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_release_create_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_gh_secret_set_allowed_tools(
    ...
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_secret_set_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh secret set:*)` tool access",
    )
}

pub(crate) fn check_gh_variable_set_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_variable_set_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(gh variable set:*)` tool access",
    )
}

pub(crate) fn check_gh_workflow_run_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.gh_workflow_run_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_sudo_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.sudo_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(sudo:*)` authority",
    )
}

pub(crate) fn check_rm_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.rm_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(rm:*)` authority",
    )
}

pub(crate) fn check_chmod_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.chmod_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
        "frontmatter grants `Bash(chmod:*)` authority",
    )
}

pub(crate) fn check_chown_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.chown_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
...
pub(crate) fn check_su_allowed_tools(
    ctx: &ScanContext,
    signals: &ArtifactSignals,
    meta: RuleMetadata,
) -> Vec<Finding> {
    findings_for_spans(
        ...
        signals
            .markdown()
            .map(|signals| signals.su_allowed_tools_spans.as_slice())
            .unwrap_or(&[]),
