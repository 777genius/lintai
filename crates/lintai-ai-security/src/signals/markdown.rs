...
use super::markdown_region_scan::{
    scan_blockquote_region, scan_code_block_region, scan_prose_region,
};
use super::shared::{common::*, markdown::*};
...
impl MarkdownSignals {
    pub(super) fn from_context(ctx: &ScanContext, metrics: &mut SignalWorkBudget) -> Option<Self> {
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Bash(*)")
        {
            signals.wildcard_bash_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        {
            signals.gh_api_delete_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        {
            signals.gh_api_patch_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh api --method PUT:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        {
            signals.gh_api_put_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh repo delete:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Bash(gh repo delete:*)")
        {
            signals.gh_repo_delete_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh repo edit:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Bash(gh repo edit:*)")
        {
            signals.gh_repo_edit_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh release delete:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh secret set:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Bash(gh secret set:*)")
        {
            signals.gh_secret_set_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh variable set:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        {
            signals.gh_variable_set_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh workflow run:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        {
            signals.gh_workflow_run_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh secret delete:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        {
            signals.gh_secret_delete_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh variable delete:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "Bash(gh workflow disable:*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
        ...
        if matches!(
            ...
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Read(*)")
        {
            signals.wildcard_read_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Write(*)")
        {
            signals.wildcard_write_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Edit(*)")
        {
            signals.wildcard_edit_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Glob(*)")
        {
            signals.wildcard_glob_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "Grep(*)")
        {
            signals.wildcard_grep_allowed_tools_spans.push(Span::new(
                region.span.start_byte + relative.start_byte,
                region.span.start_byte + relative.end_byte,
            ));
        }
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "WebFetch(*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "WebFetch(*)")
        {
            signals
        ...
        if matches!(
            ...
            && frontmatter_has_exact_allowed_tool(frontmatter_value, "WebSearch(*)")
            && let Some(region) = ctx
                .document
                .regions
                .iter()
            ...
            && let Some(snippet) = span_text(&ctx.content, &region.span)
            && let Some(relative) =
                find_exact_allowed_tool_frontmatter_relative_span(snippet, "WebSearch(*)")
        {
            signals
