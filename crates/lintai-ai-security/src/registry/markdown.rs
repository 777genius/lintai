...
pub(crate) const RULE_SPECS: [NativeRuleSpec; 70] = [
    ...
    NativeRuleSpec {
        ...
        detection_class: DetectionClass::Structural,
        lifecycle: RuleLifecycle::Stable {
            rationale: "Checks AI-native markdown for `npm` and `pnpm` install examples that point dependency resolution at `http://` registries.",
            malicious_case_ids: &["skill-npm-http-registry"],
            benign_case_ids: &["skill-npm-https-registry-safe"],
            requires_structured_evidence: true,
            remediation_reviewed: true,
            deterministic_signal_basis: "MarkdownSignals exact `npm install` or `pnpm add/install` token analysis with `--registry http://` detection inside parsed markdown regions.",
        },
    ...
    NativeRuleSpec {
        ...
        detection_class: DetectionClass::Structural,
        lifecycle: RuleLifecycle::Stable {
            rationale: "Checks AI-native markdown for `npm` and `pnpm` install examples that fetch a direct package source over `http://`.",
            malicious_case_ids: &["skill-npm-http-source"],
            benign_case_ids: &["skill-npm-https-source-safe"],
            requires_structured_evidence: true,
            remediation_reviewed: true,
            deterministic_signal_basis: "MarkdownSignals exact `npm install` or `pnpm add/install` token analysis with direct `http://` source detection inside parsed markdown regions, excluding `--registry http://` forms already covered by SEC450.",
        },
