...
## 25) Policy-resolved builtin presets (зафиксировано)

- В v1 пресеты — только **builtin policy bundles**; user-defined preset objects в `lintai.toml` не поддерживаются.
- Конфиг использует отдельный top-level блок `[presets]` с `enable = ["recommended", ...]`.
- Базовая модель:
  - builtin rule registry = source of truth
  - project config = policy selection + local overrides
  - resolved policy = единственная effective truth перед запуском engine
- Canonical builtin presets v1:
  - `recommended` — quiet practical default
  - `base` — explicit compatibility baseline
  - `strict` — `recommended` + preset-level hardening overrides
  - `compat` — transition/future-compat lane
  - `preview` — явный opt-in для Preview rules
  - `skills`, `mcp`, `claude` — builtin overlays по surface/provider intent
- `recommended` несёт curated practical default coverage; `base` остаётся узким compatibility baseline; surface overlays могут дополнительно опт-инить surface-specific preview checks.
- Preset membership for shipped rules lives in the shipped rule catalogs as source-of-truth metadata; engine/docs/explain surfaces resolve from that catalog data instead of re-deriving membership independently.
- `[[overrides]]` не меняют состав включённых preset-ов; они могут менять только category/rule severity для совпавших файлов.
- Category overrides не должны неявно активировать rule, которого нет в resolved preset set.
- Явный `[rules] RULE = "..."` может использоваться как осознанный per-rule opt-in даже для rule вне текущего preset set.
- `[extends]` остаётся отдельным future механизмом inheritance/distribution и не участвует в builtin preset resolution.
- Precedence фиксируется так:
  1. builtin rule defaults
  2. builtin group/preset defaults
  3. root `[categories]`
  4. root `[rules]`
  5. matching `[[overrides]]`
  6. CLI
