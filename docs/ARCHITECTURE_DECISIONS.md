...
- Конфиг использует отдельный top-level блок `[presets]` с `enable = ["base", ...]`.
- Базовая модель:
...
- Canonical builtin presets v1:
  ...
  - `strict` — `base` + preset-level hardening overrides
...
- `base` несёт только default stable coverage; surface overlays могут дополнительно опт-инить surface-specific preview checks.
