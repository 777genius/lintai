...
- negative safety guidance such as `never use curl ... | sh`
- abstract detector syntax such as `curl X | bash` without a concrete remote source
- markdown without a matching remote download-and-execute pattern

## False Positives

This rule lives in `threat-review` because onboarding docs, exploit walkthroughs, and lab material sometimes mention risky commands. Negative guidance and generic detector examples are excluded; concrete remote install instructions remain findings. A finding should still be read as "this document teaches a remote download-and-execute step", not "this repository is definitely compromised."
