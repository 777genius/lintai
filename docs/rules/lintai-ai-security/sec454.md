...
`SEC454` flags AI-native markdown when an `npm install` or `pnpm add/install` example fetches a package directly from `http://...`.
...
- a parsed markdown region contains `npm install`, `npm i`, `pnpm install`, or `pnpm add`
